...
\ **注意：**\ 大多数视口、光标和编辑方法都包含 ``caret_index`` 参数以支持 :ref:`caret_multiple<class_TextEdit_property_caret_multiple>`\ 。该参数应为以下之一：\ ``-1`` 用于所有光标，\ ``0`` 用于主光标，大于 ``0`` 按照其创建顺序用于辅助光标。
...
返回给定索引处的边栏的类型。边栏可以包含图标、文本或自定义视觉效果。选项见 :ref:`GutterType<enum_TextEdit_GutterType>`\ 。
...
为边栏设置自定义的绘制方法。回调方法必须接受以下参数：\ ``line: int, gutter: int, Area: Rect2``\ 。仅当边栏类型为 :ref:`GUTTER_TYPE_CUSTOM<class_TextEdit_constant_GUTTER_TYPE_CUSTOM>` 时才有效（请参阅 :ref:`set_gutter_type<class_TextEdit_method_set_gutter_type>`\ ）。
...
设置边栏的类型。边栏可以包含图标、文本或自定义视觉效果。有关选项，请参阅 :ref:`GutterType<enum_TextEdit_GutterType>`\ 。
